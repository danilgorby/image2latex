...
class LatexProducer(object):
    ...
    def _bs_decoding(self, img):
        ...
        enc_outs, (h_t, c_t) = self.model.encode(img)  # [1, H*W, OUT_C]
        B, H, W, _ = enc_outs.shape
        enc_outs = enc_outs.contiguous().view(1, H * W, -1)

        # prepare data for decoding
        enc_outs = enc_outs.expand(self.beam_size, -1, -1)
        # [Beam_size, dec_rnn_h]
        enc_outs = enc_outs.reshape(self.beam_size, H, W, -1)
        dec_states, O_t = self.model.init_decoder(enc_outs.reshape(self.beam_size, H, W, -1), (h_t, c_t))
        ...
        result = self._idx2formulas(seq.unsqueeze(0))
        return result
